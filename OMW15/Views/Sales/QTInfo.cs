...
namespace OMW15.Views.Sales
{
	public partial class QTInfo : Form
	{
		...
		//public Shared.OMShareSaleEnum.QoutationStatus QTStatus
		//{
		//	get;
		//	set;
		//}
		...
		private void CalQuotationValues(decimal QTLineTotalValues)
		{
			...
			_totalAmount = _netQTValues + _packingCost + _shippingCost;
		...
		private void GetQuotationLines(int QuotationId)
		{
			...
			//{
			//	this.CalQuotationLineValues();
			//	this.CalQuotationValues(Convert.ToDecimal(this.txtQTLineTotal.Text));
			//}
		...
		private void UpdateQuotation(int QTHeaderId)
		{
			...
			// debug -- QUOTATION NUMBER
			//MessageBox.Show(string.Format("{0}{1}{2} --- {3}", _qt.QT_PREFIX ,_qt.QT_NUMBER, (_qt.QT_REVISION == 0 ? "" : "/R"+_qt.QT_REVISION.ToString()),_qt.QT_COUNT.ToString()), "DEBUG");
			// end debug

...
namespace OMW15.Master.MasterController
{
	public class CustomerSearch 
	{
		...
		public DataTable GetCustomerList(CustomerSearchOptions SearchOption, string SearchKey)
		{
			...
			var omCustomer = (from c in _om.CUSTOMER1
							  select new 
							  {
								  ...
								  c.CUSTCODE,
								  c.CUSTNAME
							  }).Where(x => SearchOption == CustomerSearchOptions.SearchByCustomerCode ? x.CUSTCODE.Contains(SearchKey) : x.CUSTNAME.Contains(SearchKey)).AsParallel();
				...
				var _cust = (from _adb in _erp.ADDRBOOKs
							 ...
							 }).Where(x => SearchOption == CustomerSearchOptions.SearchByCustomerCode ? x.CustomerCode.Contains(SearchKey) : x.CustomerName.Contains(SearchKey)).AsParallel();

...
namespace OMW15.Casting.CastingView
{
	public partial class CastingJobs : Form
	{
		...
		private void GetActiveSummaryJob()
		{
			...
			DataTable _dt = new Casting.CastingController.JobDAL().GetSummaryJobActive();
		...
		private void GetJobOrderList(string JobCategory, int JobStatus)
		{
			...
			{
				DataTable _dt = new OMW15.Casting.CastingController.JobDAL().GetJobOrderList(JobCategory, JobStatus);
		...
		{
			this.tscbxPrintCastingForm.ComboBox.DataSource = new Casting.CastingController.JobDAL().GetJobReportForm();
		...
		{
			using (OMW15.Casting.CastingView.CastingJobInfo _jobInfo = new CastingJobInfo())
			{
		...
		private void PrintJobOrder(int SelectPrintForm, int JobNo, int JobStatus, string CustomerCode, string JobCode)
		{
			...
			Casting.CastingView.CastingReportView _crv = new CastingReportView();
		...
		private void UpdateJobOrderInfos()
		{
			...
			_resultUpdate = new Casting.CastingController.JobDAL().UpdateYearPeriodForJobInfos();

			// update status
			this.tslbUpdateJobInfos.Text = string.Format("JobInfos({0})", _resultUpdate);

			// update JobOrders for status of finishQty
			_resultUpdate = new  Casting.CastingController.JobDAL().UpdateJobOrdersStatusByFinishQty();
		...
		private void dgv_CellEnter(object sender, DataGridViewCellEventArgs e)
		{
			...
			// display picture
			this.pic.Image = OMW15.Casting.CastingController.PriceListDAL.GetPriceListItemPictureByItemNo(_itemNo);
		...
		{
			using (OMW15.Casting.CastingView.CastingJobFG _fg = new CastingJobFG())
			{
		...
		{
			using (OMW15.Casting.CastingView.JobOrderProgressStatus _jud = new JobOrderProgressStatus())
			{

...
namespace OMW15.Services.ServiceView
{
	public partial class MCHistory : Form
	{
		...
		private void GetMachineModelList()
		{
			...
			this.tscbxModel.ComboBox.DataSource = Products.ProductController.ProductCore.GetProdcutModelList();
		...
		{
			DataTable _dtCust = new Products.ProductController.ProductCore().GetCustomerListByMachine(MachineModel, SerialNo);

			if (_dtCust.Rows.Count > 0)
			{
				if (_dtCust.Rows.Count > 1)
				{
					using (Services.ServiceView.MachineOwnerList _owners = new MachineOwnerList(_dtCust))
					{
		...
		private void GetOrderByOwner(string OwnerCode, string MachineModel, string SerialNo)
		{
			...
			this.dgv.DataSource = new Services.ServiceController.ServiceJobListController().GetOrderListByMachine(OwnerCode, MachineModel, SerialNo);
		...
		private void CreateHTMLReport(int OrderId, string OrderCode)
		{
			...
			ORDERMAINTENANCE _order = new Services.ServiceController.ServiceJobInfoController().GetJobOrderInfo(OrderId);

			DataTable _dtFixed = new Services.ServiceController.ServiceJobInfoController().GetJobOrderFixedItems(OrderCode, OrderId);
			...
			DataTable dt = new Warehouse.WarehouseContoller.IssueController().GetSparePartItems(OrderId);
